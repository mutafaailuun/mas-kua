<template>
	...
	>
		...
		<template v-if="jenis === 'tercatat_kepolisian'">
			...
			>
				<tbody>
					...
					<tr>
						<td style="padding: 1px 0 1px 2.5em; vertical-align: top">
							Alamat
						</td>
						<td style="padding: 1px 8px; vertical-align: top">:</td>
						<td style="white-space: pre-line">
							{{ form.alamat_suami || "__________" }}
						</td>
					</tr>
				</tbody>
			</table>
		...
		>
			...
			yang telah melangsungkan akad nikah secara sah menurut agama Islam, dan
			pernikahan tersebut telah tercatat dalam Buku Register Nikah Kantor Urusan
			Agama Kecamatan Pebayuran Kabupaten Bekasi.
		</div>

		<!-- tercatat_terlambat -->
		<div
			v-if="jenis === 'tercatat_terlambat'"
			style="
				text-align: justify;
				text-indent: 2.5em;
				line-height: 1.8;
		...
		>
			...
			yang telah melangsungkan akad nikah secara sah menurut agama Islam, dan
			pernikahan tersebut telah tercatat dalam Buku Register Nikah Kantor Urusan
			Agama Kecamatan Pebayuran Kabupaten Bekasi. Kutipan Akta Nikah atas nama
			yang bersangkutan belum dapat diterbitkan dikarenakan proses penerbitan
			buku nikah mengalami keterlambatan.
		</div>

		<!-- tidak_tercatat -->
		<div
			v-if="jenis === 'tidak_tercatat'"
			style="
				text-align: justify;
				text-indent: 2.5em;
				line-height: 1.8;
			...
			"
		>
			Bahwa pasangan tersebut di atas telah melangsungkan akad nikah secara sah
			menurut agama Islam, namun pernikahan tersebut
			<strong>tidak tercatat</strong> dalam Buku Register Nikah Kantor Urusan
			Agama Kecamatan Pebayuran Kabupaten Bekasi.
		</div>
		...
		<template v-if="jenis === 'tidak_tercatat_isbat'">
			<div
				...
				"
			>
				Bahwa pasangan tersebut di atas telah melangsungkan akad nikah secara
				sah menurut agama Islam, namun pernikahan tersebut
				<strong>tidak tercatat</strong> dalam Buku Register Nikah Kantor Urusan
				Agama Kecamatan Pebayuran Kabupaten Bekasi.
			</div>
			<div
		...
		<template v-if="jenis === 'tidak_tercatat_bpjs'">
			...
			<div
				...
				"
			>
				Surat keterangan ini diterbitkan untuk keperluan pendaftaran
		...
		<template v-if="jenis === 'tidak_tercatat_jht'">
			...
			<div
				...
				"
			>
				Surat keterangan ini diterbitkan untuk keperluan persyaratan
				administratif klaim
				<strong>Jaminan Hari Tua (JHT) BPJS Ketenagakerjaan</strong> atas nama
				yang bersangkutan.
			</div>
		</template>
		...
		<template v-if="jenis === 'buku_nikah_palsu'">
			<div
				...
				"
			>
				Bahwa setelah dilakukan penelitian dan pengecekan pada Buku Register
				Nikah Kantor Urusan Agama Kecamatan Pebayuran,
				<strong>Nomor Akta Nikah {{ form.nomor_akta || "______" }}</strong>
				atas nama
				...
				<strong>tidak terdaftar</strong> dalam Buku Register Nikah KUA Kecamatan
				Pebayuran Kabupaten Bekasi.
			</div>
			<div
				v-if="form.nama_ppn_asli || form.nama_ppn_palsu"
				style="
					text-align: justify;
					text-indent: 2.5em;
					line-height: 1.8;
				...
				"
			>
				Pegawai Pencatat Nikah (PPN) yang tercatat dalam dokumen tersebut atas
				nama
				<strong>{{ form.nama_ppn_palsu || "______" }}</strong
				>, sedangkan PPN yang bertugas di KUA Kecamatan Pebayuran pada periode
		...
		>
			...
			>, pernikahannya tercatat di kantor KUA Kecamatan Pebayuran, dengan nomor
			akta nikah: <strong>{{ form.nomor_akta || "______" }}</strong
