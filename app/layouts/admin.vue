<template>
	<div class="min-h-screen bg-gray-50 flex">
		...
		>
			<!-- Logo + toggle -->
			<div
				class="h-16 flex items-center border-b border-gray-200 px-3 shrink-0"
				:class="collapsed ? 'justify-center' : 'justify-between px-5'"
			>
				<h1
					v-if="!collapsed"
			...
			<!-- Nav -->
			<nav
				class="flex-1 py-4 space-y-1 overflow-hidden"
			...
			>
				<div
					v-if="!collapsed"
					class="bg-emerald-50 border border-emerald-200 rounded-lg p-3 mb-3"
				>
					<p
						class="text-xs font-semibold text-emerald-700 mb-1 flex items-center gap-1"
					>
				...
				<div v-else class="flex justify-center group relative mb-2">
					<button
					...
					<div
						class="absolute left-full ml-2 z-50 px-3 py-2 text-xs font-medium text-white bg-gray-800 rounded-md whitespace-nowrap opacity-0 pointer-events-none group-hover:opacity-100 transition-opacity leading-relaxed"
					>
