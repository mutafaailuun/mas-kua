<template>
	<div>
		...
		<div class="sm:flex sm:items-center sm:justify-between mb-6">
			<div>
				...
				<p class="mt-1 text-sm text-gray-500">
					Upload dan kelola foto dokumentasi setiap peristiwa nikah.
				</p>
			</div>
		...
		>
			<div v-if="loading" class="p-10 text-center text-gray-400">
				<Icon
					...
					class="w-7 h-7 animate-spin mx-auto mb-2"
				/>
				Memuat data...
			</div>
			<div
						...
						>
							...
							<td class="px-2 py-1.5">
								<input
									...
									@keydown.escape.prevent="
										(e) => {
											(e.target as HTMLInputElement).value = wedding.groom_name;
							...
							<td class="px-2 py-1.5">
								<input
									...
									@keydown.escape.prevent="
										(e) => {
											(e.target as HTMLInputElement).value = wedding.bride_name;
											(e.target as HTMLInputElement).blur();
										}
									"
							...
							<td class="px-4 py-3 text-center">
								<span
									...
									:class="
										uploadingId === wedding.id
											? 'bg-blue-50 text-blue-500'
											: (photoCounts[wedding.id] ?? 0) > 0
							...
							<td class="px-3 py-3 text-right">
								<button
									...
									:class="{
										'text-indigo-600 bg-indigo-50': openMenuId === wedding.id,
									}"
								>
			...
			<!-- Pagination -->
			<div
				v-if="filteredWeddings.length > pageSize"
				class="border-t border-gray-100 px-4 py-3 flex items-center justify-between text-sm text-gray-600"
			>
				<span
					>Menampilkan {{ startIndex + 1 }}–{{ endIndex }} dari
					{{ filteredWeddings.length }}</span
				>
				<div class="flex gap-1">
					<button
						@click="page--"
						:disabled="page === 1"
						class="px-2 py-1 rounded border border-gray-200 disabled:opacity-40 hover:bg-gray-50"
					>
		...
		<Teleport to="body">
			...
			>
				<div
					class="flex items-center justify-between px-5 py-3 bg-white border-b border-gray-200 shrink-0"
				>
					<div class="text-sm font-medium text-gray-800">
						Preview — {{ previewWedding.groom_name }} &
						{{ previewWedding.bride_name }}
					</div>
					<div class="flex items-center gap-2">
						...
						>
							<Icon
								:name="
									exportingId === previewWedding.id
										? 'lucide:loader-2'
										: 'lucide:image'
								"
								class="w-3.5 h-3.5"
								:class="{ 'animate-spin': exportingId === previewWedding.id }"
							/>
							Export JPG
						</button>
						<button
				...
				<div
					class="flex-1 overflow-auto flex items-start justify-center py-8 px-4"
				>
					<div
						v-if="previewLoading"
						class="text-white flex items-center gap-2 mt-20"
					>
			...
			>
				...
				>
					...
					>
						<Icon
							:name="
								uploadingId === openMenuWedding.id
									? 'lucide:loader-2'
									: 'lucide:upload'
							"
							class="w-4 h-4 text-emerald-500 shrink-0"
							:class="{ 'animate-spin': uploadingId === openMenuWedding.id }"
						/>
						{{
							(photoCounts[openMenuWedding.id] ?? 0) > 0
								? "Ganti Foto"
								: "Upload Foto"
						}}
					</button>
					...
					>
						<Icon
							:name="
								exportingId === openMenuWedding.id
									? 'lucide:loader-2'
									: 'lucide:image'
							"
							class="w-4 h-4 text-amber-500 shrink-0"
							:class="{ 'animate-spin': exportingId === openMenuWedding.id }"
						/>
						Export JPG
					</button>
		...
		<Teleport to="body">
			...
			>
				<div
					class="bg-white rounded-xl shadow-xl w-full max-w-xl max-h-[80vh] flex flex-col"
				>
					<div
					...
					>
						<div>
							...
							<p class="text-xs text-gray-400 mt-0.5">
								Hanya menampilkan peristiwa nikah yang dilaksanakan di kantor
							</p>
						</div>
						<button
							@click="showKantorModal = false"
					...
					<div class="px-5 py-4 border-b border-gray-100 shrink-0">
						<div class="grid grid-cols-2 gap-3">
							<div>
								...
								<select
									v-model="kantorMonth"
									class="block w-full px-3 py-2 text-sm border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500"
								>
									<option
										v-for="(name, idx) in BULAN_INDO"
										:key="idx"
					...
					<div class="flex-1 overflow-y-auto px-5 py-3">
						...
						<div v-else>
							<label
								class="flex items-center gap-2 py-2 border-b border-gray-100 mb-2 cursor-pointer"
							>
								<input
									type="checkbox"
									:checked="kantorSelected.length === kantorWeddings.length"
									:indeterminate="
										kantorSelected.length > 0 &&
										kantorSelected.length < kantorWeddings.length
									"
									@change="toggleSelectAllKantor"
									class="h-4 w-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500"
								/>
								<span class="text-sm font-medium text-gray-700"
									>Pilih Semua ({{ kantorWeddings.length }} peristiwa)</span
								>
							</label>
							<label
								v-for="w in kantorWeddings"
								:key="w.id"
							...
							>
								...
								<div class="flex-1 min-w-0">
									...
									<div class="text-xs text-gray-500 truncate">
										{{ w.bride_name }} · {{ formatDate(w.wedding_date) }}
									</div>
								</div>
								<span
									class="text-xs px-2 py-0.5 rounded-full bg-blue-100 text-blue-700 shrink-0"
									>Kantor</span
								>
							</label>
						</div>
					...
					<div
						class="px-5 py-4 border-t border-gray-100 flex items-center justify-between shrink-0"
					>
						<span class="text-sm text-gray-500"
							>{{ kantorSelected.length }} dipilih</span
						>
						<div class="flex gap-2">
							...
							<button
								@click="executeKantorExport"
								:disabled="kantorSelected.length === 0 || kantorExporting"
							...
							>
								<Icon
									:name="kantorExporting ? 'lucide:loader-2' : 'lucide:printer'"
									class="w-4 h-4"
									:class="{ 'animate-spin': kantorExporting }"
								/>
		...
		<Teleport to="body">
			...
			>
				...
				>
							...
							>
								<input
									...
									:indeterminate="
										...
										bulkSelected.length < bulkWeddings.length
									"
									@change="toggleSelectAll"
									class="h-4 w-4 rounded border-gray-300 text-emerald-600 focus:ring-emerald-500"
								/>
								<span class="text-sm font-medium text-gray-700"
									>Pilih Semua ({{ bulkWeddings.length }} peristiwa)</span
								>
							</label>
							<label
					...
					>
						<span class="text-sm text-gray-500"
							>{{ bulkSelected.length }} dipilih</span
						>
						<div class="flex gap-2">
							<button
								@click="showBulkModal = false"
							...
							<button
								@click="executeBulkJpgExport"
								:disabled="bulkSelected.length === 0 || bulkJpgExporting"
							...
							>
								<Icon
									:name="
										bulkJpgExporting ? 'lucide:loader-2' : 'lucide:archive'
									"
									class="w-4 h-4"
									:class="{ 'animate-spin': bulkJpgExporting }"
								/>
								{{
									bulkJpgExporting
										? `Mengekspor ${bulkJpgProgress}/${bulkSelected.length}...`
										: "Export JPG (ZIP)"
								}}
							</button>
							<button
							...
							>
								<Icon
									:name="bulkExporting ? 'lucide:loader-2' : 'lucide:printer'"
									class="w-4 h-4"
									:class="{ 'animate-spin': bulkExporting }"
								/>
...
const openMenuWedding = computed(
	() => weddings.value.find((w) => w.id === openMenuId.value) ?? null,
);
...
const availableYears = computed(() => {
	const years = new Set(
		weddings.value.map((w) =>
			new Date(w.wedding_date + "T00:00:00").getFullYear(),
		),
	);
...
const filteredWeddings = computed(() => {
	...
	return weddings.value.filter((w) => {
		...
		if (filterMonth.value !== "" && d.getMonth() + 1 !== filterMonth.value)
			return false;
		if (filterYear.value !== "" && d.getFullYear() !== filterYear.value)
			return false;
		return true;
	});
});
...
const totalPages = computed(() =>
	Math.max(1, Math.ceil(filteredWeddings.value.length / pageSize)),
);
const startIndex = computed(() => (page.value - 1) * pageSize);
const endIndex = computed(() =>
	Math.min(startIndex.value + pageSize, filteredWeddings.value.length),
);
const paginatedWeddings = computed(() =>
	filteredWeddings.value.slice(startIndex.value, endIndex.value),
);

watch([searchQuery, filterMonth, filterYear, filterStatus, filterDate], () => {
	page.value = 1;
});
...
const bulkWeddings = computed(() =>
	weddings.value
		.filter((w) => {
			const d = new Date(w.wedding_date + "T00:00:00");
			return (
				d.getMonth() + 1 === bulkMonth.value &&
				d.getFullYear() === bulkYear.value
			);
		})
		.sort((a, b) => a.wedding_date.localeCompare(b.wedding_date)),
);

watch([bulkMonth, bulkYear], () => {
	bulkSelected.value = [];
});
...
const kantorWeddings = computed(() =>
	weddings.value
		.filter((w) => {
			const d = new Date(w.wedding_date + "T00:00:00");
			return (
				d.getMonth() + 1 === kantorMonth.value &&
				d.getFullYear() === kantorYear.value &&
				w.status === "Kantor"
			);
		})
		.sort((a, b) => a.wedding_date.localeCompare(b.wedding_date)),
...
const REQUIRED_FIELDS: WeddingField[] = [
	"groom_name",
	"bride_name",
	"wedding_date",
	"status",
...
const handleFileChange = async (e: Event) => {
	...
	try {
		...
		alert(
			`Gagal mengupload foto: ${err?.data?.message ?? err?.message ?? "Periksa konfigurasi R2 di .env"}`,
		);
	} finally {
...
const deletePhoto = async (wedding: any) => {
	if (
		!confirm(
			`Hapus foto dokumentasi ${wedding.groom_name} & ${wedding.bride_name}?`,
		)
	)
		return;
...
const exportJpg = async (wedding: any) => {
	...
	let photos =
		previewWedding.value?.id === wedding.id ? previewPhotos.value : [];
	if (photos.length === 0) {
		try {
			photos = await fetchPhotosForWedding(wedding.id);
		} catch {
			photos = [];
		}
	}
...
const toggleSelectAllKantor = () => {
	kantorSelected.value =
		kantorSelected.value.length === kantorWeddings.value.length
			? []
...
const executeKantorExport = async () => {
	...
	try {
		const selected = kantorWeddings.value.filter((w) =>
			kantorSelected.value.includes(w.id),
		);
...
const toggleSelectAll = () => {
	bulkSelected.value =
		bulkSelected.value.length === bulkWeddings.value.length
			? []
...
const executeBulkExport = async () => {
	...
	try {
		const selected = bulkWeddings.value.filter((w) =>
			bulkSelected.value.includes(w.id),
		);
		const pages = await Promise.all(
			selected.map(async (wedding) => {
				const photos = await fetchPhotosForWedding(wedding.id).catch(() => []);
...
const buildPageHtml = (wedding: any, photos: any[]) => {
	...
	const officiant_name =
		wedding.officiant_name || "___________________________";
